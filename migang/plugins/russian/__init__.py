...
async def _(event: GroupMessageEvent):
    ...
    if (
        ...
        await accept.finish(
            Message(
                f'这场对决是邀请 {MessageSegment.at(rs_player[event.group_id]["at"])}的，不要捣乱！'
            ),
            at_sender=True,
        )
    ...
    user_money = await UserProperty.get_gold(user_id=event.user_id)
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if rs_player[event.group_id]["at"] == event.user_id:
        at_player = await bot.get_group_member_info(
            group_id=event.group_id, user_id=event.user_id
        )
        at_player_name = at_player.get("card") or at_player["nickname"]
        await accept.send(
            Message(
                f"{MessageSegment.at(rs_player[event.group_id][1])}\n"
...
async def _(
    ...
    if msg:
        ...
        if len(msg) == 1:
            ...
            if is_number(msg) and not (int(msg) < 1 or int(msg) > 6):
                state["bullet_num"] = int(msg)
            if is_number(money) and 0 < int(money) <= await get_config(
                ...
                state["money"] = 200
                await russian.send(
                    f"赌注金额超过限制（{await get_config('max_russian_bet_gold')}），已改为200（默认）"
                )
    ats = []
...
async def _(
    ...
    state: T_State,
    bullet_num: str = ArgStr("bullet_num"),
):
    global rs_player
    ...
    if money > await get_config("max_russian_bet_gold"):
        await russian.finish(
            f"太多了！单次金额不能超过{await get_config('max_russian_bet_gold')}！",
            at_sender=True,
        )
    if money > user_money:
    ...
    if at_:
        ...
        try:
            at_player = await bot.get_group_member_info(
                group_id=event.group_id, user_id=at_
            )
            at_player_name = at_player.get("card") or at_player["nickname"]
        except AttributeError:
            at_player_name = MessageSegment.at(at_)
...
async def _(bot: Bot, event: GroupMessageEvent):
    ...
    if rs_player[event.group_id]["next"] != event.user_id:
        ...
        user_info = await bot.get_group_member_info(
            group_id=event.group_id, user_id=int(rs_player[event.group_id]["next"])
        )
        await shot.finish(
            f"你的左轮不是连发的！该 " f'{user_info.get("card") or user_info["nickname"]} 开枪了'
        )
...
async def end_game(bot: Bot, event: GroupMessageEvent):
    ...
    async with create_session() as session:
        await RussianUser.add_count(win_user_id, event.group_id, "win", session)
        await RussianUser.add_count(lose_user_id, event.group_id, "lose", session)
        await RussianUser.money(
            win_user_id, event.group_id, "win", money - fee, session
        )
        await RussianUser.money(lose_user_id, event.group_id, "lose", money, session)
        await UserProperty.modify_gold(
            win_user_id, gold_diff=money - fee, description="俄罗斯轮盘胜利"
        )
        await UserProperty.modify_gold(
            lose_user_id, gold_diff=-money, description="俄罗斯轮盘输了"
        )
        win_user = await session.scalar(
            select(RussianUser).where(
                RussianUser.user_id == win_user_id,
                RussianUser.group_id == event.group_id,
            )
        )
        lose_user = await session.scalar(
            select(RussianUser).where(
                RussianUser.user_id == lose_user_id,
                RussianUser.group_id == event.group_id,
            )
        )
    bullet_str = ""
...
async def _(event: GroupMessageEvent):
    async with create_session() as session:
        user = await session.scalar(
            select(RussianUser).where(
                RussianUser.user_id == event.user_id,
                RussianUser.group_id == event.group_id,
            )
        )
        if not user:
            user = RussianUser(user_id=event.user_id, group_id=event.group_id)
            session.add(user)
...
async def _(
    ...
    async with create_session() as session:
        if cmd[0] in ["胜场排行", "胜利排行"]:
            rank_image = await rank(bot, event.group_id, "win_rank", num, session)
        if cmd[0] in ["败场排行", "失败排行"]:
            rank_image = await rank(bot, event.group_id, "lose_rank", num, session)
        if cmd[0] == "欧洲人排行":
            rank_image = await rank(bot, event.group_id, "make_money", num, session)
        if cmd[0] == "慈善家排行":
            rank_image = await rank(bot, event.group_id, "spend_money", num, session)
        if cmd[0] == "最高连胜排行":
            rank_image = await rank(
                bot, event.group_id, "max_winning_streak", num, session
            )
        if cmd[0] == "最高连败排行":
            rank_image = await rank(
                bot, event.group_id, "max_losing_streak", num, session
            )
    if rank_image:
        await russian_rank.send(MessageSegment.image(rank_image))
