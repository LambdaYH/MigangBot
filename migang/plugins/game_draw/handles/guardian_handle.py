...
class GuardianHandle(BaseHandle[GuardianData]):
    ...
    def draw(self, count: int, pool_name: str, **kwargs) -> Message:
        ...
        return (
            pool_info
            + MessageSegment.image(self.generate_img(cards).save_png())
            + result
        )
    ...
    def generate_card_img(self, card: GuardianData) -> BuildImage:
        ...
        bg = BuildImage.new(
            "RGBA", (block_w + sep_w * 2, block_h + sep_h * 2), color="#F6F4ED"
        )
        block = BuildImage.new("RGBA", (block_w, block_h), color=block_color)
        star_path = str(self.img_path / star_name)
        star = BuildImage.open(star_path).resize((star_w, star_h))
        img_path = str(self.img_path / f"{cn2py(card.name)}.png")
        img = BuildImage.open(img_path).resize((img_w, img_h))
        block.paste(img, (0, 0), alpha=True)
        if frame_path:
            frame = BuildImage.open(frame_path).resize((img_w, img_h))
