...
class AzurHandle(BaseHandle[AzurChar]):
    ...
    def draw(self, count: int, **kwargs) -> Message:
        ...
        return MessageSegment.image(self.generate_img(cards).save_png()) + result
    ...
    def generate_card_img(self, card: AzurChar) -> BuildImage:
        ...
        w = 100
        h = 100
        bg = BuildImage.new("RGBA", (w + sep_w * 2, h + sep_t + sep_b))
        frame_path = str(self.img_path / f"{card.star}_star.png")
        frame = BuildImage.open(frame_path).resize((w, h))
        img_path = str(self.img_path / f"{cn2py(card.name)}.png")
        img = BuildImage.open(img_path).resize((w, h))
        # 加圆角
    ...
    async def _update_info(self):
        ...
        char_list = dom.xpath(
            "//div[@class='mw-body-content mw-content-ltr']/div[@class='mw-parser-output']/div[@id='CardSelectTr']/div"
        )
        for char in char_list:
            try:
                name = char.xpath("./span[@class='jntj-4']/a/@title")[0]
                frame = char.xpath(
                    "./div[@class='jntj-2']/div[@class='jntj-3']/a/img/@alt"
                )[0]
                avatar = char.xpath("./div[@class='jntj-2']/img/@src")[0]
            except IndexError:
                continue
            member_dict = {
