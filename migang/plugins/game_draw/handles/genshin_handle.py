...
class GenshinHandle(BaseHandle[GenshinData]):
    ...
    def generate_card_img(self, card: GenshinData) -> BuildImage:
        ...
        img_h = 106
        bg = BuildImage.new(
            "RGBA", (frame_w + sep_w * 2, frame_h + sep_h * 2), color="#EBEBEB"
        )
    ...
    def draw(self, count: int, user_id: int, pool_name: str = "", **kwargs) -> Message:
        ...
        bk = BuildImage.new("RGBA", (img.width, img.height + 50), color="#ebebeb")
        bk.paste(img)
        bk.draw_text(
            (0, img.height + 10),
            "【五星：0.6%，四星：5.1%，第72抽开始五星概率每抽加0.585%】",
        ...
        return pool_info + MessageSegment.image(bk.save_png()) + result
