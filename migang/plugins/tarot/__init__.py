"""
哔哩哔哩主题塔罗牌占卜，移植自zhenxun_bot塔罗牌插件
"""
import random
...
tarot = on_fullmatch(("塔罗牌", "塔罗占卜", "/抽卡", "/塔罗占卜"), priority=5, block=True)
...
async def _(bot: Bot, event: MessageEvent):
    ...
    for count in range(4):
        ...
        image_file = MessageSegment.image(IMG_PATH / f"{card_key}.jpg")

        # 特殊规则：愚者有两张
        if card_key == "愚者":
            rand = random.randint(1, 2)
            image_file = MessageSegment.image(IMG_PATH / f"{card_key}{rand}.jpg")
    ...
    if isinstance(event, GroupMessageEvent):
        await bot.send_forward_msg(group_id=event.group_id, messages=chain)
    else:
        await bot.send_forward_msg(user_id=event.user_id, messages=chain)
