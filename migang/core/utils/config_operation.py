...
async def get_config(key: str, plugin_name: Optional[str] = None) -> Any:
    """异步获取插件配置，当plugin_name不指定且当前插件位于以plugins结尾的文件夹中时，插件名将默认为当前插件，最好不要在插件未完全加载时载入插件配置，否则第一次启动时会抛出异常且无法及时获取新默认值
...
def sync_get_config(key: str, plugin_name: Optional[str] = None) -> Any:
    """同步获取插件配置，当plugin_name不指定且当前插件位于以plugins结尾的文件夹中时，插件名将默认为当前插件，最好不要在插件未完全加载时载入插件配置，否则第一次启动时会抛出异常且无法及时获取新默认值
