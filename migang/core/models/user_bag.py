...
class UserBag(Model):
    ...
    @classmethod
    async def add_property(cls, user_id: int, item_name: str, amount: int = 1):
        """
        说明:
            增加道具
        参数:
            ...
            :param num: 道具数量
        """
        user, _ = await cls.get_or_create(user_qq=user_id, item_name=item_name)
        user.amount += amount
        await user.save(update_fields=["amount"])

    @classmethod
    async def delete_property(
        cls, user_id: int, item_name: str, amount: int = 1
    ) -> bool:
        """
        说明:
            使用/删除 道具
    ...
    @classmethod
    async def get_property(cls, user_id: int, item_name: str) -> bool:
    ...
    @classmethod
    async def get_property_list(cls, user_id: int):
        return await cls.filter(user_id=user_id).exclude(amount=0).order_by("ASC").all()

    @classmethod
    async def get_name_by_idx(cls, user_id: int, idx: int):
        property_list = await cls.get_property_list(user_id=user_id)
