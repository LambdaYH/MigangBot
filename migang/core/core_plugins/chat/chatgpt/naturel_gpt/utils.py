...
async def default_permission_check_func(
    matcher: Matcher, event: MessageEvent, bot: Bot, cmd: str, type: str = "cmd"
) -> Tuple[bool, str]:
    """默认权限检查函数"""
    ...
    is_super_user = str(event.user_id) in config.ADMIN_USERID or await SUPERUSER(
        bot, event
    )
    is_admin = (
        is_super_user
        or isinstance(event, PrivateMessageEvent)
        or await (GROUP_ADMIN | GROUP_OWNER)(bot, event)
    )  # 超级管理员，私聊，群主，群管理，均视为admin
                ...
                if qq:
                    if qq == "all":
                        ...
                        user_name = await get_user_name(
                            event=event, bot=bot, user_id=int(qq)
                        )
                        if user_name:
...
async def get_user_name(
    event: Union[MessageEvent, GroupIncreaseNoticeEvent], bot: Bot, user_id: int
) -> str:
    """获取QQ用户名，优先群名片"""
    if isinstance(event, GroupMessageEvent) or isinstance(
        ...
        user_info = await bot.get_group_member_info(
            group_id=event.group_id, user_id=user_id, no_cache=False
        )
    ...
    if (
        user_name and config.NG_CHECK_USER_NAME_HYPHEN and ("-" in user_name)
    ):  # 检查用户名中的连字符, 去掉第一个连字符之前的部分
