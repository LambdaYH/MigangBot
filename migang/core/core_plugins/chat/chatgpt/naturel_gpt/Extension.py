    ...
    def generate_description(self, chat_history_text="") -> str:
        ...
        if self._ext_config["refer_word"] and chat_history_text:
            for refer_word in self._ext_config["refer_word"]:
                ...
                return ""
        args_desc: str = "; ".join(
            [f"{k}:{v}" for k, v in self._ext_config.get("arguments", {}).items()]
        )
...
def load_extensions(config: Dict[str, Any]) -> None:
    ...
    if not os.path.exists("data/naturel_gpt/ext_cache"):
        os.makedirs("data/naturel_gpt/ext_cache", exist_ok=True)
    # 删除ext_cache文件夹下的所有文件和文件夹
    for file in os.listdir("data/naturel_gpt/ext_cache"):
        file_path = os.path.join("data/naturel_gpt/ext_cache", file)
        if os.path.isdir(file_path):
            shutil.rmtree(file_path)
        else:
            os.remove(file_path)
    # 在ext_cache文件夹下建立一个__init__.py文件 用于标记该文件夹为一个python包
    if not os.path.exists("data/naturel_gpt/ext_cache/__init__.py"):
        with open("data/naturel_gpt/ext_cache/__init__.py", "w", encoding="utf-8") as f:
            f.write("")

    # 根据 Extension 文件 生成 Extension.py 并覆盖到扩展模块路径和 ext_cache 文件夹下
    with open(
        os.path.join(os.path.dirname(__file__), "Extension.py"), "r", encoding="utf-8"
    ) as f:
        ext_file = f.read()
    with open(f"{ext_path}Extension.py", "w", encoding="utf-8") as f:
        f.write(ext_file)
    with open(f"data/naturel_gpt/ext_cache/Extension.py", "w", encoding="utf-8") as f:
        f.write(ext_file)
    ...
    for tmpExt in config["NG_EXT_LOAD_LIST"]:  # 遍历扩展模块列表
        if tmpExt.get("IS_ACTIVE") and tmpExt.get("EXT_NAME"):
            ...
            try:
                ...
                shutil.copyfile(
                    f"{ext_path}{file_name}", f"data/naturel_gpt/ext_cache/{file_name}"
                )
                time.sleep(0.3)  # 等待文件复制完成
                # 从 ext_cache 文件夹下导入扩展模块
                CustomExtension: Extension = getattr(
                    importlib.import_module(
                        f'data.naturel_gpt.ext_cache.{tmpExt.get("EXT_NAME")}'
                    ),
                    "CustomExtension",
                )
                ...
                ext_config_dict = (
                    tmpExt.get("EXT_CONFIG")
                    if isinstance(tmpExt.get("EXT_CONFIG"), dict)
                    else {}
                )
                ext = CustomExtension(ext_config_dict)  # 加载扩展模块并实例化
                global_extensions[
                    ext.get_config().get("name").lower()
                ] = ext  # 将扩展模块添加到全局扩展模块字典中
...
if __name__ == "__main__":
    ...
    shutil.copyfile(
        ...
        os.path.join(os.path.dirname(__file__), "..", "extensions", "Extension.py"),
    )
    # 复制 Extension.py 到 ../share_exts/ 目录下
    shutil.copyfile(
        os.path.join(os.path.dirname(__file__), "Extension.py"),
        os.path.join(os.path.dirname(__file__), "..", "share_exts", "Extension.py"),
    )
