    ...
    for tmpExt in config['NG_EXT_LOAD_LIST']:   # 遍历扩展模块列表
        if tmpExt.get('IS_ACTIVE') and tmpExt.get('EXT_NAME'):
            ...
            try:
                ...
                shutil.copyfile(f'{ext_path}{file_name}', f'data/naturel_gpt/ext_cache/{file_name}')
                time.sleep(0.3)  # 等待文件复制完成
                # 从 ext_cache 文件夹下导入扩展模块
                CustomExtension:Extension = getattr(importlib.import_module(f'data.naturel_gpt.ext_cache.{tmpExt.get("EXT_NAME")}'), 'CustomExtension')
