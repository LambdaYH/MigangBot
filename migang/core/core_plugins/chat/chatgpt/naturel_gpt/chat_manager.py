...
class ChatManager(Singleton["ChatManager"]):
    ...
    def get_or_create_chat(self, chat_key: str) -> Chat:
        ...
        if chat_key in self._chat_dict:
            ...
            # 创建新对话
            chat_data = PersistentDataManager.instance.get_or_create_chat_data(
                chat_key=chat_key
            )
            self._chat_dict[chat_key] = Chat(chat_data)
    ...
    def rename_preset_for_all(
        self, old_preset_key: str, new_preset_key: str
    ) -> Tuple[int, int]:
        """改名所有会话指定预设, 对话历史将全部丢失！"""
        success_cnt = fail_cnt = 0
        for chat in self._chat_dict.values():
            if chat.rename_preset(
                old_preset_key=old_preset_key, new_preset_key=new_preset_key
            )[0]:
                success_cnt += 1
            else:
                fail_cnt += 1
    ...
    def add_preset_for_all(
        self, preset_key: str, bot_self_introl: str
    ) -> Tuple[int, int]:
        """将预设添加到所有的会话中, 返回值为(成功数量，失败数量)"""
        success_cnt = fail_cnt = 0
        for chat in self._chat_dict.values():
            success, _ = chat.add_preset(
