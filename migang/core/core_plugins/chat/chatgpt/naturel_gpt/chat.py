...
from .persistent_data_manager import ChatData, PresetData, ImpressionData
...
class Chat:
    ...
    def __init__(self, chat_data: ChatData, preset_key: str = ""):
        if not isinstance(chat_data, ChatData):
            raise Exception(
                f"chat_data 参数不是ChatData类型,实际类型为:{type(chat_data).__name__}"
            )
    ...
    def get_chat_prompt_template(self, userid: str = None) -> str:
        ...
        if config.MEMORY_ACTIVE:  # 如果记忆功能开启
            if global_extensions.get("remember") and global_extensions.get(
                ...
                logger.debug("未加载主动记忆 memory 扩展，仅启用固定记忆！")
                memory = (
                    (f"[history memory]\n" f"{memory_text}\n") if memory_text else ""
                )
    ...
    def add_preset(self, preset_key: str, bot_self_introl: str) -> Tuple[bool, str]:
        ...
        self._chat_preset_dicts[preset_key] = PresetData(
            preset_key=preset_key, bot_self_introl=bot_self_introl
        )
        return (True, None)

    def add_preset_from_config(
        self, preset_key: str, preset_config: PresetConfig
    ) -> Tuple[bool, str]:
        """从配置添加新人格, config_preset为config中的全局配置"""
        if preset_key in self._chat_preset_dicts:
            return (False, "同名预设已存在")

        self._chat_preset_dicts[preset_key] = PresetData.create_from_config(
            preset_config
        )
        # 更新默认值
    ...
    def del_preset(self, preset_key: str) -> Tuple[bool, str]:
        ...
        default_preset_key = [
            preset for preset in self._chat_preset_dicts.values() if preset.is_default
        ][0].preset_key
    ...
    def rename_preset(
        self, old_preset_key: str, new_preset_key: str
    ) -> Tuple[bool, str]:
        """改名指定预设, 对话历史将全部丢失！"""
