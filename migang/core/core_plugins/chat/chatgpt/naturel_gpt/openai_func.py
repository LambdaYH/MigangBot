...
class TextGenerator(Singleton["TextGenerator"]):
    ...
    def get_response(self, prompt, type: str = "chat", custom: dict = {}) -> str:
        ...
        for _ in range(len(self.api_keys)):
            if type == "chat":
                res, success = self.get_chat_response(
                    self.api_keys[self.key_index], prompt, custom
                )
            elif type == "summarize":
                res, success = self.get_summarize_response(
                    self.api_keys[self.key_index], prompt, custom
                )
            elif type == "impression":
                res, success = self.get_impression_response(
                    self.api_keys[self.key_index], prompt, custom
                )
            if success:
                return res, True
            ...
            logger.warning(
                f"当前 Api Key({self.key_index}): [{self.api_keys[self.key_index][:4]}...{self.api_keys[self.key_index][-4:]}] 请求错误，尝试使用下一个..."
            )
